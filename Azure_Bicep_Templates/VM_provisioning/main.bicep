...
@description('Whitelisted IP address that can ssh to the vm.')
param sshSourceIpAddress string
    ...
    securityRules: [
      {
        ...
        properties: {
          ...
          access: 'Allow'
          direction: 'Inbound'
          sourceAddressPrefix: sshSourceIpAddress
          sourcePortRange: '*'
...
resource vmExtension 'Microsoft.Compute/virtualMachines/extensions@2025-04-01' = if (securityType == 'TrustedLaunch' && securityProfileJson.uefiSettings.secureBootEnabled && securityProfileJson.uefiSettings.vTpmEnabled) {
  parent: vm
  name: extensionName
  location: location
  properties: {
